...
int dsi_panel_set_opec_mode(struct dsi_panel *panel, int mode)
{
	...
	mutex_lock(&panel->panel_lock);
	if (!panel->spec_pdata->display_onoff_state) {
		pr_err("%s: Display is off, can't set OPEC mode\n", __func__);
		mutex_unlock(&panel->panel_lock);
		return -EAGAIN;
	}

	rc = (mode == 0) ? dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_OPEC_OFF) :
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_OPEC_ON);

	if (rc != 0) {
		pr_err("[%s] failed to send DSI_CMD_SET_OPEC_%s cmd, rc=%d\n", panel->name, mode == 0 ? "OFF" : "ON", rc);
	} else {
