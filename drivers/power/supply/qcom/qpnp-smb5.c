...
static enum power_supply_property smb5_dc_props[] = {
	...
	POWER_SUPPLY_PROP_CHG_PWR_DC_ICL,
	POWER_SUPPLY_PROP_DCIN_EN_STS,
	POWER_SUPPLY_PROP_RESET_DCIN_EN,
#endif
};
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_WIRELESS_MODE:
		break;
	case POWER_SUPPLY_PROP_CHG_PWR_DC_ICL:
		val->intval = get_client_vote(chg->dc_icl_votable,
							SOMC_CHG_PWR_VOTER);
		break;
	case POWER_SUPPLY_PROP_DCIN_EN_STS:
		rc = smblib_somc_get_prop_dcin_en_sts(chg, val);
		break;
	case POWER_SUPPLY_PROP_RESET_DCIN_EN:
		val->intval = 0;
		break;
#endif
	default:
...
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
	case POWER_SUPPLY_PROP_WIRELESS_MODE:
		rc = smblib_somc_set_prop_wireless_mode(chg, val);
		break;
	case POWER_SUPPLY_PROP_CHG_PWR_DC_ICL:
		rc = vote(chg->dc_icl_votable, SOMC_CHG_PWR_VOTER,
							true, val->intval);
		break;
	case POWER_SUPPLY_PROP_RESET_DCIN_EN:
		rc = smblib_somc_set_prop_reset_dcin_en(chg);
		break;
#endif
	default:
		return -EINVAL;
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
		...
		break;
	case POWER_SUPPLY_PROP_AICL_DONE:
		rc = smblib_get_prop_aicl_done(chg, val);
		break;
#endif
	default:
...
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
	POWER_SUPPLY_PROP_OTG_CONNECTED,
#endif
};
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHG_PWR_INDICATION_CONTROL:
		...
		break;
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
	case POWER_SUPPLY_PROP_OTG_CONNECTED:
		val->intval = chg->somc_otg_connected;
		break;
#endif
	default:
...
static int smb5_configure_typec(struct smb_charger *chg)
{
	...
	smblib_masked_write(chg, TYPEC_TYPE_C_DEBUG_ACCESS_SNK_CFG, 0xff,
			EN_DEBUG_ACCESS_SNK | EN_CHG_ON_DEBUG_ACCESS_SNK |
			DAM_DIS_AICL);
#if !defined(CONFIG_SOMC_CHARGER_EXTENSION)
...
#if defined(CONFIG_SOMC_CHARGER_EXTENSION)
	vote(chg->dc_suspend_votable, SOMC_DC_PLUGIN_VOTER, true, 0);
#endif
	return rc;
}
...
enum smb5_somc_sysfs {
	...
	ATTR_DC_SUSPEND_VOTER,
	ATTR_DC_SUSPEND_VOTER_EFFECTIVE,
	ATTR_FCC_VOTER,
	...
	ATTR_REG_USBIN_ADAPTER_ALLOW_OVERRIDE,
	ATTR_REG_USBIN_ADAPTER_ALLOW_CFG,
	ATTR_DCIN_UV_COUNT,
	ATTR_DCIN_AICL_TARGET,
	ATTR_DCIN_AICL_SETTLED,
	ATTR_DCIN_AICL_ADJUSTMENT,
	ATTR_DCUSBEX_STATUS,
...
static ssize_t smb5_somc_param_show(struct device *dev,
				...
				char *buf);
static ssize_t smb5_somc_param_store(struct device *dev,
				struct device_attribute *attr,
				...
				const char *buf, size_t count)
{
	...
	case ATTR_DCIN_AICL_SKIP:
		...
		if (ret < 0)
			return ret;
		chg->debug_dcin_aicl_skip = val == 1 ? true : false;
		break;
	default:
		break;
	}
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if ((chg->pd_active == POWER_SUPPLY_PD_PPS_ACTIVE) ||
		...
		if (rc < 0)
			dev_err(chg->dev,
				"Couldn't usbin adapter allow cfg. rc=%d\n", rc);
	}
#endif
}
